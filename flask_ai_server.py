...
# TELEGRAM BOT CONFIG
# ============================================
TELEGRAM_TOKEN = "8275068458:AAFPqpdOgkJcdGCatqqjbetdo7emeYZs0bQ"     # <== BOT TOKEN
...
# Model mặc định (nếu không truyền gì từ client)
DEFAULT_VOICE_MODEL_NAME = os.environ.get("WHISPER_MODEL", "medium").lower()

# Những model cho phép (bạn có model nào thì liệt kê vào đây)
...
COMMON_FIXES = {
    # ===== THƯƠNG HIỆU / ĐÈN =====
    "đen ện quang": "đèn điện quang",
    "ện quang": "điện quang",
    "đèn ện hoàng": "đèn điện quang",
    "ện hoàng": "điện quang",
    "ện ngoan": "điện quang",
    ...
    "financonix": "panasonic",
    "hana sonic": "panasonic",
    "hana sonic sau": "panasonic sau",

    # philips/phillips đã có, nhưng thêm vài biến thể
    "phillips": "philips",
    "philips": "philips",
    "phê lếp": "philips",
    ...
    "la post": "loa bose",
    "loa bâu": "loa bose",
    "post": "bose",          # chủ yếu dùng cho "loa post" -> "loa bose"

    "lá bài": "loa bài",
    "la bai": "loa bài",

    # ===== SCENE / CHẾ ĐỘ =====
    "sinh relax": "scene relax",
    "sinh focus": "scene focus",
    "sinh party": "scene party",
    "sinh night": "scene night",
    "mở sinh": "mở scene",
    "sin relax": "scene relax",
    "sin focus": "scene focus",
    "sin party": "scene party",
    "sin night": "scene night",
    "mở sin": "mở scene",

    "vaccine relax": "scene relax",
    "vaccine focus": "scene focus",
    "vaccine 40": "scene party",
    "vaccine 9": "scene night",

    "về mặt đỉnh": "về mặc định",
    "ve mat dinh": "về mặc định",
    "mặt đỉnh": "mặc định",

    # ===== TẮT / BẬT TẤT CẢ THIẾT BỊ =====
    "tác hết": "tắt hết",
    "tác tất cả": "tắt tất cả",
    "tác đèn": "tắt đèn",
    "tác": "tắt",  # để cuối cùng cho đè lên các trường hợp lẻ

    # Một số lỗi nhỏ khác trong log
    "tắt qu t": "tắt quạt",
    "tắt qu t dyson": "tắt quạt dyson",
    "tắt loa bông": "tắt loa bose",
    ...
    "cửa phòng": "cửa phụ",
    "cua phong": "cửa phụ",
    "mở cửa phòng": "mở cửa phụ",
    "mo cua phong": "mở cửa phụ",
    # ===== SCENE / CHẾ ĐỘ =====
    # Party
    "pati": "party",
    "pát ti": "party",
...
# Lazy init Whisper (có thể đổi model linh hoạt)
WHISPER_MODEL = None
...
def _select_model_name(requested):
    """
    Chọn tên model hợp lệ:
    - ưu tiên model client yêu cầu
    - nếu không hợp lệ thì dùng DEFAULT_VOICE_MODEL_NAME
    """
    name = (requested or DEFAULT_VOICE_MODEL_NAME or "small").strip().lower()
    if name not in ALLOWED_WHISPER_MODELS:
        print(f"Requested Whisper model '{name}' không hợp lệ, dùng '{DEFAULT_VOICE_MODEL_NAME}'.")
        name = DEFAULT_VOICE_MODEL_NAME
    return name


def _load_whisper_model(model_name):
    """
    Load Whisper model với tên cho trước.
    - Nếu model hiện tại đã đúng tên -> dùng lại, không load lại
    - Nếu tên khác -> giải phóng model cũ, load model mới
    """
    global WHISPER_MODEL, _whisper_device, _current_model_name, _whisper_inited
    ...
    if _whisper_device is None:
        if torch.cuda.is_available():
            ...
            print("Không có GPU, chạy CPU cho Whisper.")
            _whisper_device = "cpu"
        print("PyTorch:", torch.__version__, "| CUDA:", torch.cuda.is_available())

    target = _select_model_name(model_name)

    # nếu đã load đúng model rồi thì thôi
    if WHISPER_MODEL is not None and _current_model_name == target:
    ...
    t0 = time.time()
    print(f"Đang tải Whisper model '{target}' trên thiết bị '{_whisper_device}' ...")
    try:
        WHISPER_MODEL = whisper.load_model(target, device=_whisper_device)
    except Exception as e:
    ...
    _whisper_inited = True
    print(f"Whisper model sẵn sàng: {target} on {_whisper_device} (load {time.time() - t0:.2f}s)")
    return True


def _init_whisper_safe(requested_model=None):
    """
    Hàm wrapper dùng trong /voice:
    - requested_model là tên model client yêu cầu (hoặc None)
    """
    return _load_whisper_model(requested_model)
...
def _parse_command_vi(text: str) -> dict:
    ...
    if "cua" in sf or "door" in sf:
        # fix thêm trường hợp "cua phong" mà Whisper hay trả về
        if "cua phong" in sf:
            sf = sf.replace("cua phong", "cua phu")

        # cửa trước / cửa chính
        if any(k in sf for k in ("cua truoc", "cua chinh", "cua vao", "front door")):
            door_id = "frontDoor"
        # cửa bên / cửa hông / cửa phụ / cửa sau / cửa phòng
        elif any(
            k in sf
            for k in (
                "cua ben",
    ...
    # ----- map mức → speed/volume cho quạt & loa -----
    ...
    if is_fan:
        # ƯU TIÊN: "tốc độ số 3", "mức số 2", "mức 3"
        # sf đã được _fold_text → "toc do so 3" / "muc so 3"
        m_speed = re.search(r"(?:toc do|muc)\s*(?:so\s*)?(\d+)", sf)
        if m_speed:
            try:
                sp = int(m_speed.group(1))
            except Exception:
                sp = None
            if sp is not None:
                # map trực tiếp 1..3
                speed = max(0, min(3, sp))
                # không dùng brightness nữa, tránh hiểu nhầm 3% → speed 1
                brightness = None
                brightness_delta = None

        # Nếu không nói “tốc độ / mức số N” thì mới fallback sang %
        if speed is None and brightness is not None:
            if brightness <= 0:
                speed = 0
            else:
                speed = max(1, min(3, round(brightness * 3 / 100)))
            brightness = None  # đã dùng để suy ra speed

        if speed is None and brightness_delta is not None:
    ...
    if is_speaker:
        # tuyệt đối: "loa Bose 30%" → volume = 30
        if brightness is not None:
            volume = brightness
            # không để lại brightness cho loa, tránh kéo theo đèn/quạt
            brightness = None

        # tương đối: "tăng/giảm âm lượng loa 20%" → volume = 20 (hoặc bước mặc định)
        if brightness_delta is not None:
            ...
            # không cho delta lan sang đèn/quạt
            brightness_delta = None
...
def voice():
    # Lấy JSON (nếu có) một lần
    data_json = request.get_json(silent=True) or {}

    # Lấy tên model từ client (header / form / json)
    requested_model = (
        request.headers.get("X-Whisper-Model")
        or request.form.get("model")
        or data_json.get("model")
    )

    # Lazy init Whisper với model yêu cầu
    if not _init_whisper_safe(requested_model):
    ...
    try:
        ...
        if "audio" in request.files:
            ...
            model_name_log = _current_model_name or requested_model or DEFAULT_VOICE_MODEL_NAME

            print(
                f" Transcribe start (device={_whisper_device}, model={model_name_log}, execute={execute})"
            )
        ...
        if (
            ...
            if execute and cmds:
                if due_ts_all:
                    ...
                    for c in cmds:
                        try:
                            # Với lệnh "tất cả" dùng thẳng desired để tránh nháy
                            results.append(_push_desired_from_voice(c))
                        except Exception as e_push_all:
                            print("ALL_DEVICES_OFF push error:", e_push_all)
        ...
        # ========= ALL DEVICES ON =========
        if (
            "bat tat ca thiet bi" in sf_all
            or "bat het tat ca thiet bi" in sf_all
            or "bat het thiet bi" in sf_all
            or ("bat tat ca" in sf_all and "thiet bi" in sf_all)
        ):
            print("🔌 ALL DEVICES ON intent:", sf_all)

            try:
                r_devs = requests.get(FIREBASE_DEVICES_URL, timeout=4)
                devs = r_devs.json() or {}
            except Exception as e_all_on_dev:
                print("⚠️ ALL_DEVICES_ON: không đọc được /devices:", e_all_on_dev)
                devs = {}
            ...
            print(f"🔎 ALL_DEVICES_ON: tìm được {len(cmds)} thiết bị.")

            # delay kiểu 'sau 5 giây', 'trong 2 phút'
            delay_all = _extract_delay_vi(text)
            if delay_all:
                for c in cmds:
                    c["delay_seconds"] = delay_all
                print(f"⏱ ALL_DEVICES_ON delay {delay_all} giây cho {len(cmds)} thiết bị.")

            # scheduler kiểu 'sau 5 giây', 'lúc 7 giờ'
            due_ts_all, due_label_all = _parse_schedule_vi(text)
            ...
            if execute and cmds:
                if due_ts_all:
                    ...
                    print(
                        f"🗓 ALL_DEVICES_ON scheduled {len(cmds)} cmds at {due_label_all} (ts={due_ts_all})"
                    )
                else:
                    for c in cmds:
                        try:
                            ...
                            print("⚠️ ALL_DEVICES_ON push error:", e_push_on)

            multi_cmd = {
                "multi": cmds,
                "action": "all_devices_on",
                "raw_text": text,
            }
            return jsonify(
                {
        ...
        if (
            ...
            if execute and cmds:
                if due_ts_all:
                    ...
                    # KHÔNG có thời gian → tắt ngay lập tức, đi thẳng desired
                    for c in cmds:
                        try:
                            results.append(_push_desired_from_voice(c))
                        except Exception as e_push:
                            print("ALL_OFF push error:", e_push)
        ...
        # ========= ALL LIGHTS ON =========
            ...
            print("ALL_ON: tìm được", len(cmds), "thiết bị đèn.")

            # parse delay kiểu "sau 5 giây", "trong 2 phút"
            delay_all = _extract_delay_vi(text)
            if delay_all:
                for c in cmds:
                    c["delay_seconds"] = delay_all
                print("ALL_ON delay", delay_all, "giây cho", len(cmds), "thiết bị.")

            # dùng scheduler để hẹn giờ (hỗ trợ cả 'sau 5 giây', 'lúc 7 giờ')
            due_ts_all, due_label_all = _parse_schedule_vi(text)
            ...
            if execute and cmds:
                if due_ts_all:
                    # CÓ thời gian cụ thể → chỉ schedule, không đẩy ngay
                    scheduled = []
                    for c in cmds:
                        job_id = _schedule_command(due_ts_all, c, sandbox=sandbox)
                        scheduled.append(
                    ...
                    # KHÔNG có thời gian → bật ngay lập tức
                    for c in cmds:
                        try:
                            results.append(_push_command(c, sandbox=sandbox))
                        except Exception as e_push_on:
                            print("ALL_ON push error:", e_push_on)
        ...
        if execute and not due_ts and (
            ...
            print("DRY-RUN (không thực thi).")
        else:
            print(" Đã xếp lịch, không đẩy ngay.")
...
def _push_desired_from_voice(cmd: dict) -> dict:
    try:
        ...
        dev_type = str(cmd.get("device") or "").lower()
        is_light = (
            ("light" in dev_type)
            or ("den" in dev_type)
            or ("lamp" in dev_type)
            or ("neopixel" in dev_type)
        )
        ...
        # ---- ON / OFF (áp dụng cho mọi loại thiết bị) ----
        if action in ("on", "off"):
            patch["power"] = action
        
        # ---- CHỈ ĐÈN MỚI DÙNG brightness / màu / rainbow ----
        if is_light:
            # Brightness tuyệt đối
            if brightness is not None:
                try:
                    b = int(brightness)
                    b = max(0, min(100, b))
            ...
            if color_name:
                ...
                hex_color = LIGHT_COLOR_HEX.get(key)
            ...
            # Nếu tắt đèn → luôn về 0 và tắt rainbow
            if action == "off":
                patch["brightness"] = 0
                patch["rainbow"] = False

        # ---- QUẠT / LOA: chỉ speed / volume, KHÔNG dùng brightness/color/effect ----
        ...
        if volume is not None:
            try:
                ...
                # Đồng bộ state(bool) với power / speed / volume
        if "power" in patch:
            if patch["power"] == "on":
                patch["state"] = True
            elif patch["power"] == "off":
                patch["state"] = False
        else:
            # Nếu không set power nhưng có speed/volume thì suy ra power
            if speed is not None:
                try:
                    s_val = int(speed)
                except Exception:
            ...
            if volume is not None and "power" not in patch:
                try:
                    v_val = int(volume)
                except Exception:
                    v_val = None
