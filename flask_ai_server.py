...
def _load_device_catalog(force: bool=False):
    ...
    for key, dev in (data.items() if isinstance(data, dict) else []):
        name  = str(dev.get("name") or key)
        area  = str(dev.get("area") or dev.get("room") or "Default")
        dtype = str(dev.get("device") or dev.get("type") or "Light")
        brand = str(dev.get("brand") or "")
        ...
        if (
            ...
            if isinstance(devs, dict):
                for key, dev in devs.items():
                    ...
                    dev_id = str(dev.get("id") or key)
                    area = str(dev.get("area") or "Default")
