...
def _best_device_match(text: str, area_hint: str = None):
    """
    Chọn thiết bị tốt nhất dựa trên:
    - fuzzy match với syn/name/device
    - ưu tiên khu vực (area_hint)
    - ưu tiên từ khóa đặc biệt: neopixel, dien quang
    - lọc đúng loại: đèn / quạt / loa / cửa
    """
    cat = _load_device_catalog()
    if not cat:
        return None
    ...
    # Từ khóa đặc biệt trong câu nói
    wants_neopixel = any(k in sf for k in ("neopixel", "neo pixel", "neo"))
    wants_dien_quang = ("dien quang" in sf) or ("dienquang" in sf)

    # Ý định loại thiết bị
    want_light = any(k in sf for k in ("den", "light", "neopixel"))
    want_fan = ("quat" in sf) or ("fan" in sf)
    want_speaker = ("loa" in sf) or ("speaker" in sf) or ("am thanh" in sf)
    want_door = ("cua" in sf) or ("door" in sf)
    ...
    for d in cat:
        ...
        syn_list = d.get("syn") or []
        syn_f = [_fold_text(s) for s in syn_list]

        profile = " ".join([name_f, dev_f, area_f, " ".join(syn_f)])

        is_neopixel = any(k in profile for k in ("neopixel", "neo pixel", "neo"))
        is_dien_quang = ("dien quang" in profile) or ("dienquang" in profile)
        ...
        candidates.append(
            {
                **d,
                ...
                "_syn_f": syn_f,
                "_profile": profile,
                "_is_neopixel": is_neopixel,
                "_is_dien_quang": is_dien_quang,
                "_is_light": is_light,
                "_is_fan": is_fan,
                "_is_speaker": is_speaker,
                "_is_door": is_door,
            }
        )

    # Lọc theo loại thiết bị nếu người dùng nói rõ
    def _category_ok(c):
        if want_light and not (c["_is_light"] or c["_is_neopixel"]):
            return False
        if want_fan and not c["_is_fan"]:
            return False
        if want_speaker and not c["_is_speaker"]:
            return False
        if want_door and not c["_is_door"]:
            return False
        return True

    filtered = [c for c in candidates if _category_ok(c)] or candidates

    # Nếu có nhắc rõ "neopixel" → ưu tiên / lọc NeoPixel
    if wants_neopixel:
        neos = [c for c in filtered if c["_is_neopixel"]]
        if neos:
            filtered = neos

    # Nếu có nhắc "điện quang" → ưu tiên thiết bị Điện Quang
    if wants_dien_quang:
        dqs = [c for c in filtered if c["_is_dien_quang"]]
        if dqs:
            filtered = dqs
    ...
    for d in filtered:
        # dùng syn + name + device để tính điểm
        syns = d["_syn_f"] + [
            d["_name_f"],
            d["_dev_f"],
            f'{d["_dev_f"]} {d["_area_f"]}',
        ]
        ...
        if area_hint_f and d["_area_f"] == area_hint_f:
            score *= 1.15

        # buff thêm nếu trùng từ khóa đặc biệt
        if wants_neopixel and d["_is_neopixel"]:
            score *= 1.6
        if wants_dien_quang and d["_is_dien_quang"]:
            score *= 1.6

        if score > best_score:
            best, best_score = d, score

    # hạ ngưỡng chút cho dễ bắt
...
def voice():
    ...
    try:
        ...
        if (
            ...
            # 🔴 parse delay cho lệnh "tắt tất cả"
            delay_all = _extract_delay_vi(text)
            if delay_all:
                for c in cmds:
                    c["delay_seconds"] = delay_all
